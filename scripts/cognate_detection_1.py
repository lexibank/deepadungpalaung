...
input('loaded data')
part.get_partial_scorer(runs=100, threshold=1.0) # make tests with 100 and 1000, when debugging)
part.partial_cluster(method='lexstat', threshold=0.55, ref='cogids', cluster_method='infomap')
alms = Alignments(part, ref='cogids')
alms.align()
alms.output('tsv', filename='../output/deepadung-wordlist-1.0', ignore='all', prettify=False)
